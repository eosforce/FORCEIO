...
class whitelist_blacklist_tester {
   public:
      ...
      void init( bool bootstrap = true ) {
         ...
         chain->set_code(N(eosio.token), force_token_wast);
         chain->set_abi(N(eosio.token), force_token_abi);
...
BOOST_AUTO_TEST_CASE( contract_whitelist ) { try {
   ...
   test.chain->set_code(N(bob), force_token_wast);
   test.chain->set_abi(N(bob), force_token_abi);

   test.chain->produce_blocks();

   test.chain->set_code(N(charlie), force_token_wast);
   test.chain->set_abi(N(charlie), force_token_abi);
...
BOOST_AUTO_TEST_CASE( blacklist_eosio ) { try {
   ...
   tester1.chain->set_code(config::system_account_name, force_token_wast);
   tester1.chain->produce_blocks();
   tester1.shutdown();
